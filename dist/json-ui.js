...
;angular.module('jcab.json-ui').directive('jsonAddValue', ['jsonDefaultValue', function(jsonDefaultValue) {
  return {
    restrict: 'A',
...
;angular.module('jcab.json-ui').directive('jsonBoolean', function() {
  return {
    restrict: 'A',
    templateUrl: 'json-ui/boolean.tpl.html'
  };
});
...
;angular.module('jcab.json-ui').directive('jsonPushValue', ['jsonDefaultValue', function(jsonDefaultValue) {
  return {
    restrict: 'A',
...
;angular.module('jcab.json-ui').factory('recursiveDirectiveHelper', ['$compile', function($compile) {
...
;angular.module('jcab.json-ui').run(['$templateCache', function($templateCache) {  'use strict';
  ...
  $templateCache.put('json-ui/add-value.tpl.html',
    "<form ng-submit=\"addProp(propType)\" class=\"form-inline text-right\"><input ng-model=\"propName\" class=\"form-control form-control-sm\" placeholder=\"Nom du champ\" style=\"width: 240px\"><select ng-model=\"propType\" class=\"form-control form-control-sm\"><option value=\"string\">Texte</option><option value=\"number\">Nombre</option><option value=\"bool\">Booléen</option><option value=\"array\">Tableau</option><option value=\"object\">Objet</option></select><button type=\"submit\" class=\"invisible\">ok</button></form>"
  );


  $templateCache.put('json-ui/array.tpl.html',
    "<table class=\"table table-bordered\"><tr ng-repeat=\"(key, value) in objData track by $index\"><td valign=\"top\" class=\"json-label\">{{ key }}:</td><td valign=\"top\" class=\"json-value\" json-value=\"objData[key]\"></td></tr><tr><td colspan=\"2\" class=\"json-add-value text-right\" json-push-value></td></tr></table>"
  );


  $templateCache.put('json-ui/boolean.tpl.html',
    "<span class=\"json-field json-boolean\"><input type=\"checkbox\" ng-model=\"objData[key]\"></span>"
  );
  ...
  $templateCache.put('json-ui/number.tpl.html',
    "<span class=\"json-field json-number\"><input type=\"number\" ng-model=\"objData[key]\" class=\"form-control\"></span>"
  );


  $templateCache.put('json-ui/object.tpl.html',
    "<table class=\"table table-bordered\"><tr ng-repeat=\"(key, value) in objData\"><td valign=\"top\" class=\"json-label\"><span>{{ key }}:</span></td><td valign=\"top\" class=\"json-value\"><span json-value=\"objData[key]\"></span></td></tr><tr><td colspan=\"2\" class=\"json-add-value text-right\" json-add-value></td></tr></table>"
  );


  $templateCache.put('json-ui/push-value.tpl.html',
    "<form ng-submit=\"pushValue(propType)\" class=\"form-inline text-right\"><select ng-model=\"propType\" class=\"form-control form-control-sm\" ng-change=\"pushValue(propType)\"><option value=\"\">Ajouter</option><option value=\"string\">Texte</option><option value=\"number\">Nombre</option><option value=\"bool\">Booléen</option><option value=\"array\">Tableau</option><option value=\"object\">Objet</option></select><span ng-if=\"objData.length > 0\" style=\"color: white\">ou <button type=\"button\" ng-click=\"pushLastClone()\" class=\"btn btn-secondary btn-sm\">Dupliquer le dernier élément</button></span> <button type=\"submit\" class=\"invisible\">ok</button></form>"
  );
  ...
  $templateCache.put('json-ui/value.tpl.html',
    "<span ng-if=\"isString(data)\" json-string=\"data\"></span> <span ng-if=\"isNumber(data)\" json-number=\"data\"></span> <span ng-if=\"isBoolean(data)\" json-boolean=\"data\"></span> <span ng-if=\"isArray(data)\" json-array=\"data\"></span> <span ng-if=\"isObject(data)\" json-object=\"data\"></span>"
  );
}]);